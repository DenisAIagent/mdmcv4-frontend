...
const Reviews = () => {
  ...
  // Fonction pour générer les étoiles (affichage)
  ...
  const submitReview = async (e) => {
    ...
    const apiUrl = import.meta.env.VITE_API_URL; // Assure-toi que VITE_API_URL est bien définie dans Railway (frontend)
    if (!apiUrl) {
    ...
    try {
      ...
      // Pas de closeForm ici, peut-être juste vider les champs après succès ?
      setName(''); setEmail(''); setRating(0); setMessage('');
      setTimeout(() => setFormSuccess(null), 5000); // Efface le message succès après 5s
    } catch (error) {
    ...
    <section id="reviews" className="reviews-section">
      <div className="container">
        ...
        <div className="reviews-actions">
           {/* Le bouton 'Laisser un avis' n'ouvre plus de modal ici */}
         ...
         <div style={{ border: '2px solid red', padding: '20px', marginTop: '30px' }}> {/* Pour bien le repérer */}
               ...
               <div className="rating-selector">
                 {[1, 2, 3, 4, 5].map((starValue) => (
                   <span
                     ...
                     onClick={() => !isSubmitting && handleRatingClick(starValue)} // Garde le log dans la fonction
