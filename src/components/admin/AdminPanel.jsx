...
// Assurez-vous que les chemins sont corrects par rapport à l'emplacement de AdminPanel.jsx
...
const AdminPanel = () => {
  ...
  const [isLoadingReviews, setIsLoadingReviews] = useState(false); // Renommé pour clarté
  ...
  const fetchPendingReviews = async () => {
    ...
    if (!apiUrl) { console.error("VITE_API_URL non définie"); setFetchError("Erreur config API."); setIsLoadingReviews(false); return; }
    try {
      const response = await fetch(`${apiUrl}/reviews?status=pending`); // Assurez-vous que cet endpoint est protégé si besoin
      ...
      else { setFetchError("Format réponse API invalide pour les avis."); }
    } catch (error) { console.error("Erreur fetch avis:", error); if (!fetchError) { setFetchError("Impossible charger avis."); } }
    finally { setIsLoadingReviews(false); }
  };

  useEffect(() => {
    // Charger les données initiales pour le dashboard ou l'onglet reviews si c'est l'onglet actif
    if (activeTab === 'dashboard' || activeTab === 'reviews') {
        fetchPendingReviews();
    }
    // Vous pourriez avoir d'autres chargements de données ici en fonction de activeTab
  }, [activeTab]);

  const handleLogout = () => {
    // Idéalement, utilisez votre AuthContext ou service d'authentification ici
    // auth.logout().then(() => window.location.href = '/admin/login');
    localStorage.removeItem('mdmc_admin_auth'); // Adaptez si la clé du token est différente
  ...
  const updateReview = async (id, newStatus) => {
    // ... (votre logique existante pour updateReview)
  ...
  const handleNavigateToCreateSmartlink = () => {
    ...
    // Pas besoin de setActiveTab('smartlinks') ici, car on est déjà dans la section smartlinks
  };
  ...
  const handleSmartlinkFormSuccess = () => {
    setSmartlinkView('list'); // Revenir à la liste après succès
    setEditingSmartlinkId(null);
    // Optionnel: Forcer le re-chargement de la liste si SmartlinkListPage ne le fait pas
  };
  
  const handleChangeTab = (tabName) => {
    setActiveTab(tabName);
    // Si on quitte l'onglet smartlinks ou si on y entre, on réinitialise à la vue 'list'
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'smartlinks': // NOUVEL ONGLET
        if (smartlinkView === 'create') {
          // SmartlinkCreatePage reçoit onFormSubmitSuccess pour revenir à la liste
          return <SmartlinkCreatePage onFormSubmitSuccess={handleSmartlinkFormSuccess} />;
        } else if (smartlinkView === 'edit' && editingSmartlinkId) {
          // SmartlinkEditPage reçoit smartlinkId et onFormSubmitSuccess
          ...
          // SmartlinkListPage reçoit des callbacks pour changer smartlinkView en 'create' ou 'edit'
  ...
  return (
    <div className={`admin-panel ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <AdminChatbot /> {/* S'il est toujours utilisé */}
      ...
      <div className="admin-sidebar">
        ...
        <div className="admin-nav">
          ...
          <button className={`nav-item ${activeTab === 'smartlinks' ? 'active' : ''}`} onClick={() => handleChangeTab('smartlinks')}>
            ...
            <span className="nav-text">{t('admin.smartlinks_nav_button') || 'SmartLinks'}</span> {/* Clé de traduction suggérée */}
          </button>
          {/* --- FIN NOUVEAU BOUTON --- */}
