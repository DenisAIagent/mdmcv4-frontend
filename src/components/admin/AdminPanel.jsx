...
// N'UTILISEZ PAS useNavigate ici si AdminPanel gère la navigation interne pour cette section
import {
  Box,
  Typography,
  Button,
...
import apiService from '@/services/api.service'; // Assurez-vous que cet alias est configuré dans vite.config.js
...
function SmartlinkListPage({ onNavigateToCreate, onNavigateToEdit }) {
  ...
  const fetchSmartlinks = useCallback(async () => {
    ...
    setError(null);
    try {
      // Utilise la méthode getAll de smartLinkService dans apiService
  ...
  const handleEditClick = (id) => {
    if (onNavigateToEdit) {
      ...
      console.error("SmartlinkListPage: onNavigateToEdit prop is not defined. Cannot navigate to edit.");
    }
  };
  ...
  const handleCreateClick = () => {
    if (onNavigateToCreate) {
      ...
      console.error("SmartlinkListPage: onNavigateToCreate prop is not defined. Cannot navigate to create.");
    }
  }
  ...
  // CORRECTION: L'annotation de type TypeScript ': GridColDef[]' a été enlevée.
  const columns = [
    {
    ...
    <Paper sx={{ p: { xs: 1, sm: 2, md: 3 }, width: '100%', overflow: 'hidden', borderRadius: 2, boxShadow: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 1 }}>
        ...
        <Button
          ...
          color="primary"
          startIcon={<AddIcon />}
          onClick={handleCreateClick} // Utilise la fonction de callback
        >
          Nouveau SmartLink
        </Button>
      </Box>
      <Box sx={{ height: 'calc(100vh - 250px)', minHeight: 450, width: '100%' }}>
        <DataGrid
          rows={smartlinks}
          columns={columns}
          loading={loading}
