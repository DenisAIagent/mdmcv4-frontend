...
// CORRECTION DU CHEMIN D'IMPORTATION POUR ADMIN.CSS
import '../../assets/css/admin.css'; // Chemin corrigé
...
// IMPORTS POUR LE MODULE SMARTLINK
// Ces chemins supposent que AdminPanel.jsx est dans src/components/admin/
// et les pages SmartLink dans src/pages/admin/smartlinks/
...
const AdminPanel = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'dashboard') {
        fetchPendingReviews(); // Le dashboard affiche aussi les stats des reviews
    }
  ...
  const updateReview = async (id, newStatus) => {
    setActionMessageReviews({ type: '', text: '' });
    if (!VITE_API_URL) { setActionMessageReviews({ type: 'error', text: "Erreur config API." }); return; }
    const token = localStorage.getItem('mdmc_admin_auth');
    const headers = { 'Content-Type': 'application/json', };
    if (token) { headers['Authorization'] = `Bearer ${token}`; }

    try {
      const response = await fetch(`${VITE_API_URL}/reviews/${id}`, { method: 'PUT', headers: headers, body: JSON.stringify({ status: newStatus }) });
      ...
      setActionMessageReviews({ type: 'success', text: `Avis ${newStatus === 'approved' ? 'approuvé' : 'rejeté'} !` });
      setTimeout(() => setActionMessageReviews({ type: '', text: '' }), 3000);
    } catch (error) { console.error(`Erreur màj avis ${id}:`, error); setActionMessageReviews({ type: 'error', text: error.message || "Erreur màj." }); }
  };
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'smartlinks') {
      return (
        <section id="smartlinks" className="admin-smartlinks active">
          {/* Le titre "Gestion des SmartLinks" sera dans SmartlinkListPage ou ici si besoin */}
    ...
    if (activeTab === 'content') return ( <section id="content" className="admin-content-section active"> <h2>{t('admin.content_management')}</h2> <p>Contenu...</p> </section> );
    if (activeTab === 'media') return ( <section id="media" className="admin-media active"> <h2>{t('admin.media_management')}</h2> <p>Contenu...</p> </section> );
    ...
    <div className={`admin-panel ${sidebarCollapsed && !isMobile ? 'sidebar-collapsed' : ''}`}>
      ...
      <nav className={`admin-sidebar ${isMobile && sidebarCollapsed ? 'mobile-sidebar-hidden' : isMobile && !sidebarCollapsed ? 'mobile-sidebar-visible' : ''}`}>
        ...
        <div className="admin-nav">
          ...
          {/* ... autres boutons ... */}
           <button className={`nav-item ${activeTab === 'content' ? 'active' : ''}`} onClick={() => handleChangeTab('content')} data-tab="content">
          ...
          <button className={`nav-item ${activeTab === 'media' ? 'active' : ''}`} onClick={() => handleChangeTab('media')} data-tab="media">
          ...
          <div className="nav-divider" style={{height: "1px", backgroundColor: "rgb(74, 85, 104)", margin: "1rem 0px"}}></div>
          <button className={`nav-item ${activeTab === 'marketing-integrations' ? 'active' : ''}`} onClick={() => handleChangeTab('marketing-integrations')} data-tab="marketing-integrations">
          ...
          <button className={`nav-item ${activeTab === 'wordpress' ? 'active' : ''}`} onClick={() => handleChangeTab('wordpress')} data-tab="wordpress">
          ...
          <button className={`nav-item ${activeTab === 'wordpress-sync' ? 'active' : ''}`} onClick={() => handleChangeTab('wordpress-sync')} data-tab="wordpress-sync">
          ...
          <button className={`nav-item ${activeTab === 'landing-pages' ? 'active' : ''}`} onClick={() => handleChangeTab('landing-pages')} data-tab="landing-pages">
          ...
          <button className={`nav-item ${activeTab === 'settings' ? 'active' : ''}`} onClick={() => handleChangeTab('settings')} data-tab="settings">
